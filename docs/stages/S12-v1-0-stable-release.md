...
- [ ] ADR-0015 implemented (issue #15): no frozen version module's
      wire language depends on a shared mutable enum (`Outcome` /
      `ValuePolicy` are version-local or mapped from frozen
      primitives). Hard blocker — v1.0 must not ship with this hole in
