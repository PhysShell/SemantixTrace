...
//!   one. v1 ships with an identity chain; the trait surface is fixed
//!   from day one so the v1 → v2 transition is purely additive.
...
//! the bump procedure when a v2 lands.
...
/// consumers.
pub type Current = v1::TraceEvent;
...
pub fn write_event(event: &Current) -> Result<String, SchemaError> {
    let envelope = v1::TraceEnvelope::from_event(event.clone());
